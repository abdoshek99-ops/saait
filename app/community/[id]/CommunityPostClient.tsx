  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white" dir="rtl"
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* بطاقة الموضوع */}
        <div className="bg-[#0a0a12] border border-purple-900/20 rounded-3xl p-7 mb-6
        ...
        {/* قسم التعليقات */}
        <div className="bg-[#0a0a12] border border-purple-900/20 rounded-3xl p-7
                        shadow-[0_0_40px_rgba(124,58,237,0.03)]">
          ...
          {session ? (
            <div className="mb-6">
              <textarea
                ...
                rows={3}
                className="w-full bg-[#13131f] text-white rounded-xl px-4 py-3
                           border border-gray-700 focus:border-purple-500
                           focus:outline-none resize-none placeholder-gray-600 text-sm mb-3"
                placeholder="اكتب تعليقك هنا..." />
              <button onClick={handleComment} disabled={submitting || !comment.trim()}
                className="bg-purple-600 hover:bg-purple-700 disabled:opacity-50
                           text-white font-bold px-6 py-2.5 rounded-xl transition text-sm">
            ...
            <div className="space-y-4">
              {comments.map((c: any) => (
                <div key={c.id} className="flex gap-3">
                  <div className="w-9 h-9 rounded-full bg-purple-800 flex items-center justify-center text-sm font-bold flex-shrink-0 overflow-hidden">
                    {c.author?.image ? (
                      <img src={c.author.image} alt="" className="w-9 h-9 object-cover" />
                    ) : (
