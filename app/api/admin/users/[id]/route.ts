  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.message.deleteMany({
        where: { OR: [{ senderId: params.id }, { receiverId: params.id }] }
      })
      // حذف عضوية المشاريع
      await tx.projectMember.deleteMany({ where: { userId: params.id } })
      // حذف المشاريع المملوكة
      await tx.project.deleteMany({ where: { ownerId: params.id } })
      // حذف المواضيع
      await tx.post.deleteMany({ where: { authorId: params.id } })
      // حذف الإشعارات
      await tx.notification.deleteMany({ where: { userId: params.id } })
      // حذف الأفكار
      await tx.idea.deleteMany({ where: { authorId: params.id } })
      // حذف تسجيلات الفعاليات
      await tx.eventRegistration.deleteMany({ where: { userId: params.id } })
      // حذف الشارات
      await tx.userBadge.deleteMany({ where: { userId: params.id } })
      // حذف المهارات
      await tx.userSkill.deleteMany({ where: { userId: params.id } })
      // حذف رموز التحقق
      await tx.verificationToken.deleteMany({ where: { email: user.email } })
      // حذف الملف الشخصي
      await tx.profile.deleteMany({ where: { userId: params.id } })
      // حذف المستخدم أخيراً
